...
def get_metadata():
    hg = HeteroData()
    hg['hw'].x = torch.randn(100, 3)
    hg['para'].x = torch.randn(10, 3)
    fout, fin = gen_in_out_edge(100, 10)
    hg['hw', 'fout', 'para'].edge_index = fout 
    hg['para', 'fin', 'hw'].edge_index = fin
    hg['para', 'connect1', 'para'].edge_index = gen_edge(10)
    hg['hw', 'connect2', 'hw'].edge_index = gen_edge(100)
    return hg.metadata()

def get_gso():
    adj_mx = np.load('hz_rn_adj.npy')
    adj_mx = adj_mx - np.eye(100)
...
def gen_in_out_edge(n1, n2):
    nodes1 = list(np.array([1,6,15,24,39,50,67,76,89,93]))
...
def get_supports():
    adj_mx = np.load('data/G56/hz_rn_adj.npy')
...
def load_dataset(mode="train"):
    file_path = 'data/G56/'
    hw_ptr = np.load(file_path+'G56_high.npz',allow_pickle=True)
    para_ptr = np.load(file_path+'G56_para.npz',allow_pickle=True)
    ...
    para_train_x, para_train_y, para_val_x, para_val_y, para_test_x,para_test_y = para_ptr['data']
    # scaler
