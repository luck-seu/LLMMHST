...
def parse_args():
    ...
    parser.add_argument("--seq_len", type=int, default=24*6, help="seq_len")
    parser.add_argument("--pred_len", type=int, default=24*6, help="out_len")
...
class trainer:
    ...
    def train(self, input, mark, embeddings, real):
        ...
        mse_loss = self.loss(predict, real)

        kl_input_log_probs = F.log_softmax(predict, dim=1)
        kl_target_probs = F.softmax(real, dim=1)
        
        kl_loss = self.kl_loss_fn(kl_input_log_probs, kl_target_probs, reduction='batchmean')
